...
int main(void)
{
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    HAL_Delay(500); //NOTE!!! Double breakpoint in HAL_Delay!
  }
  /* USER CODE END 2 */
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_ConfigPinAttributes(GPIOA, GPIO_PIN_13|GPIO_PIN_14, GPIO_PIN_NSEC);

  /*IO attributes management functions */
  HAL_GPIO_ConfigPinAttributes(GPIOB, GPIO_PIN_3, GPIO_PIN_NSEC);

  /*Configure GPIO pin : LD_BLUE_Pin */
  ...
  HAL_GPIO_ConfigPinAttributes(GPIOG, GPIO_PIN_7, GPIO_PIN_NSEC);
